...
class ConciliadorSAT:
    ...
    def recorreResDiariosSATIcaav(self):
        ...
        try:
            dst = self.directorio + self.agencia + "/SAT" + self.agencia[0:3] + "-" + self.ano + "-" + ''.join(str(e) for e in self.meses) + "-" + self.fecha + ".csv"
    ...
    ###Recorre Resultado Diarios y Ventas
    def recorreResDiariosVentas(self):
        nameFunction = "recorreResDiariosVentas" + self.reporteType

        try:
            dst = self.directorio + self.agencia + "/Ventas" + self.agencia[0:3] + "-" + self.ano + "-" + ''.join(str(e) for e in self.meses) + "-" + self.fecha + ".csv"
            print("voy a empezar csv")
            ...
            with open(dst, 'w') as csv_file:
                ...
                csv_file.write("Fecha VENTAS" + ",")
                csv_file.write("BASE VENTAS" + ",")
                csv_file.write("IVA VENTAS" + ",")
                csv_file.write("TUA VENTAS" + ",")
                csv_file.write("OTROS VENTAS" + ",")

                csv_file.write("Monto VENTAS" + ",")
                csv_file.write("Monto ICAAV" + ",")
                ...
                for clave in sorted(self.diariosTodo.keys()):
                    ...
                    #csv_file.write(renglon["referenciaDiario"] + ",")
                    #csv_file.write(renglon["referenciaDescripcionDiario"] + ",")
                    csv_file.write(str(renglon["fechaVenta"]) + ",")
                    csv_file.write(str(renglon["tarifaBaseVenta"]) + ",")
                    csv_file.write(str(renglon["ivaVenta"]) + ",")
                    csv_file.write(str(renglon["tuaVenta"]) + ",")
                    csv_file.write(str(renglon["otrosVenta"]) + ",")

                    csv_file.write(str(renglon["totalVenta"]) + ",")
                    csv_file.write(str(abs(renglon["montoDiario"])) + ",")

                    
                    diferencia = abs(abs(renglon["montoDiario"])-abs(renglon["totalVenta"]))
                    if("SOLO" not in renglon["status"] and diferencia >.01):
                        if(diferencia >1):
                            self.diariosTodo[clave]["status"] = "MONTOS," + str(diferencia)
                        else:
                            self.diariosTodo[clave]["status"] = "OK2" ##+ str(diferencia)
    ...
    def datosDiario(self, clave, cuentaDiario, polizaDiario, datosPolizaFechaDiario, sucursalDiario,
                    docDiario, referenciaDiario, referenciaDescripcionDiario, montoDiario):
    ...
    def datosVentas(self, clave,docDiario,fechaVenta,tarifaBaseVenta,ivaVenta, tuaVenta, otrosVenta,totalVenta):
        nameFunction = "datosVentas"

        try:
            self.diariosTodo[clave]["docVenta"] = docDiario
            self.diariosTodo[clave]["fechaVenta"] = fechaVenta
            self.diariosTodo[clave]["tarifaBaseVenta"] = tarifaBaseVenta
            self.diariosTodo[clave]["ivaVenta"] = ivaVenta
            self.diariosTodo[clave]["tuaVenta"] = tuaVenta
            self.diariosTodo[clave]["otrosVenta"] = otrosVenta
            self.diariosTodo[clave]["totalVenta"] = totalVenta
        except Exception as err:
            self.wriErr.addMensaje("error", "Error sistema: " + nameFunction + "-" + str(err))
            WriteLog.writeLog(nameFunction, err)
    ...
    def extractInfoVentas(self):
        nameFunction = "extractInfoVentas"

        try:

            for mes in self.meses:
                srcVentas = self.directorio + self.ano + "-" + str(mes).zfill(2) + "/Ventas " + self.agencia + ".csv"
                try:
                    file = open(srcVentas, "r", encoding="utf8", errors='ignore')
                except Exception as err:
                    print("El archivo " + srcVentas + " no existe.")
                    self.wriErr.addMensaje("error", "El archivo " + srcVentas + " no existe.")
                    break
                print("Voy a empezar a leer Ventas")

                for line in file:
                    if (line.split(',')[1] != "datos_factura_id_serie" and line[0:3] != ",,,"):
                        docDiario = line.split(',')[1] + " "
                        numeroDocDiario = line.split(',')[2]
                        clave = docDiario + numeroDocDiario
                        fechaVenta = line.split(',')[13]
                        tarifaBase = float(line.split(',')[7])
                        iva = float(line.split(',')[8])
                        tua = float(line.split(',')[9])
                        otros = float(line.split(',')[10])

                        if (clave in self.diariosTodo):
                            self.diariosTodo[clave]["status"] = "OK1"
                        elif (clave not in self.diariosTodo and docDiario[1] == "F"):
                            self.diariosTodo[clave] = {}
                            self.datosVentas(clave, "", "", 0, 0, 0, 0, 0)
                            self.diariosTodo[clave]["status"] = "SOLO EN VENTAS"

                        ##elif(clave in self.diariosTodo):
                        if (clave in self.diariosTodo):
                            docDiario = docDiario + "-" + self.diariosTodo[clave]["docDiario"]
                            fechaVenta = fechaVenta + "-" + self.diariosTodo[clave]["fechaVenta"]
                            tarifaBase = tarifaBase + self.diariosTodo[clave]["tarifaBaseVenta"]
                            iva = iva + self.diariosTodo[clave]["ivaVenta"]
                            tua = tua + self.diariosTodo[clave]["tuaVenta"]
                            otros = otros + self.diariosTodo[clave]["otrosVenta"]
                            total = tarifaBase + iva + tua + otros

                            ##if(self.diariosTodo[clave]["datosPolizaFechaDiario"] != ""):
                                ##self.diariosTodo[clave]["status"] = "OK1"

                            self.datosVentas(clave,docDiario,fechaVenta,tarifaBase,iva, tua, otros,total)
    ...
    ###Extracts Info to be analized
    def extractInfoDiariosIngresos(self):
        nameFunction = "extractInfoDiariosIngresos"
            ...
            for mes in self.meses:
                ...
                for line in file:
                    ...
                    if (line.split(',')[1] != "datos_poliza_anio" and line[0:3] != ",,,"):
                        ...
                        if(cuentaDiario[0:4] in ["1130"]
                           and polizaIDDiario in ["PE","PI"]                                 
                           and docDiario[0] not in ["D"]):
                            ...
                            self.diariosTodo[clave]["status"] = "SOLO EN INGRESOS"
                            
                            self.datosDiario(clave, cuentaDiario, polizaDiario, datosPolizaFechaDiario,sucursalDiario,docDiario, referenciaDiario, referenciaDescripcionDiario, montoDiario)
                            self.datosVentas(clave,"","",0,0,0,0,0)
        ...
        try:
            ...
            for mes in self.meses:
                ...
                try:
                    ...
                    self.wriErr.addMensaje("error", "El archivo " + srcDiarios + " no existe.")
                    break
                print("Voy a empezar a leer Diarios " + str(mes))
                ##WriteLog.writeLog("info", "empece a leer diarios")
        ...
        try:
            for mes in self.meses:
                ...
                try:
                    ...
                    break
                print("Voy a empezar a leer SAT" + str(mes))
